...
import {fetchInvolvedIssues, fetchInvolvedDiscussions} from '@/libs/github/issues'
...
export function useRepositoryIssuesDiscussions(year: number) {
    ...
    return useQuery({
        ...
        queryFn: async () => {
            ...
            const repoMap = new Map<string, RepositoryIssueDiscussionStats & {username: string}>()
            ...
            return Array.from(repoMap.values()).sort((a, b) => b.totalCount - a.totalCount)
        },
        enabled: !!user,
    })
}
