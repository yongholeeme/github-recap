...
export async function fetchMostDiscussedPR(
  ...
  const reviewsData = await fetcher<Array<{ id: number; body: string }>>({
  ...
  const reviewCommentsData = await fetcher<Array<{ id: number }>>({
  ...
  const reviewsWithBody = reviewsData.filter(
